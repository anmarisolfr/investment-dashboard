...
function fetchNews(ticker) {
  return new Promise((resolve) => {
    // Yahoo Finance RSS — per-ticker if provided, general market otherwise
    ...
    const options = { headers: { "User-Agent": "Mozilla/5.0", "Accept": "application/rss+xml, application/xml, text/xml" } };
    https.get(endpoint, options, (res) => {
      let data = "";
      res.on("data", (chunk) => (data += chunk));
      res.on("end", () => {
        try { resolve(parseRSS(data, 8)); }
        catch(e) { resolve([]); }
      });
    }).on("error", () => resolve([]));
  });
