...
function fetchNews(ticker) {
  return new Promise((resolve) => {
    const q = ticker ? encodeURIComponent(ticker) : encodeURIComponent('stock market');
    const endpoint = `https://query1.finance.yahoo.com/v1/finance/search?q=${q}&newsCount=8&quotesCount=0`;
    const options = { headers: { "User-Agent": "Mozilla/5.0", "Accept": "application/json" } };
    https.get(endpoint, options, (res) => {
      let data = "";
      res.on("data", (chunk) => (data += chunk));
