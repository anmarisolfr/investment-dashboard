...
function buildDesc(ticker, price, ma50, ma100, ma200, rsi, signal) {
  ...
  if (ma200) {
    ...
    if (price > ma200) {
      parts.push(`El precio está ${pct}% por encima de su promedio de los últimos 200 días — señal de tendencia positiva.`);
    } else {
      parts.push(`El precio está ${pct}% por debajo de su promedio de los últimos 200 días — el activo lleva tiempo bajando.`);
    }
  }

  // MA50 — tendencia reciente
  if (ma50) {
    if (price > ma50) {
      parts.push(`En las últimas semanas el precio sigue subiendo.`);
    } else {
      parts.push(`En las últimas semanas el precio ha estado bajando.`);
    }
  }

  // RSI — momento actual
  if (rsi <= 30) {
    parts.push(`El activo ha caído mucho y rápido — puede ser una oportunidad de compra, pero también podría seguir bajando.`);
  } else if (rsi >= 70) {
    parts.push(`El activo ha subido mucho en poco tiempo — existe riesgo de que corrija hacia abajo pronto.`);
  } else if (rsi >= 50) {
    parts.push(`El impulso es moderadamente positivo, sin señales extremas.`);
  } else {
    parts.push(`El impulso es moderadamente negativo, sin señales extremas.`);
  }

  // Señal final
  if (signal === "buy") {
    parts.push(`En resumen: buen momento para considerar este activo.`);
  } else if (signal === "caution") {
    parts.push(`En resumen: no es el mejor momento para entrar. Conviene esperar.`);
  } else {
    parts.push(`En resumen: el activo no muestra una dirección clara. Mejor observar antes de decidir.`);
  }
...
function scoreExplain(score, zone, price, ma50, ma100, ma200, rsi, dividendYield) {
  ...
  if (score >= 80) return `Este activo tiene muy buenas condiciones ahora mismo — el precio está en una tendencia positiva, el momentum es saludable y los indicadores están alineados.${divText}`;
  if (score >= 60) return `Este activo luce interesante — tiene varias señales positivas aunque no todas están perfectamente alineadas. Vale la pena seguirlo de cerca.${divText}`;
  if (score >= 40) return `Este activo está en zona neutral — ni muy bien ni muy mal. No hay una señal clara para entrar o salir ahora mismo.${divText}`;
  return `Este activo requiere precaución — el precio o el momentum muestran señales negativas. No es el mejor momento para comprar.${divText}`;
}
...
async function getTickerData(ticker) {
  ...
  // Use Yahoo's real-time daily change % instead of comparing closes
  ...
  // Cross signals — only for watchlist display
...
function categorizeTickers(tickers) {
  ...
  // Fill each category to at least 5 from defaults if not enough trending
  const defaults = {
