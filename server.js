...
function fetchYahoo(ticker) {
  return new Promise((resolve, reject) => {
    ...
    https.get(endpoint, options, (res) => {
      ...
      res.on("end", () => {
        try { resolve(JSON.parse(data)); }
        catch (e) { reject(new Error("Error parseando respuesta de Yahoo")); }
      });
    }).on("error", reject);
  });
}

// ── NOTICIAS (RSS feed de Yahoo Finance, público) ──
function parseRSS(xml, limit, publisher) {
  const items = [];
  const parts = xml.split("<item>").slice(1);
  for (const raw of parts.slice(0, limit || 8)) {
    const get = (tag) => {
      const m = raw.match(new RegExp("<" + tag + ">([\\s\\S]*?)</" + tag + ">"));
      if (!m) return null;
      return m[1].replace(/<!\[CDATA\[/g, "").replace(/\]\]>/g, "").replace(/&amp;/g,"&").replace(/&#39;/g,"'").replace(/&quot;/g,'"').trim();
    };
    ...
    if (title && link) {
      items.push({
        title,
        publisher: publisher || "Investing.com",
        link,
...
function httpsGet(endpoint, headers) {
...
async function fetchNews(ticker) {
  ...
  // 1) Intentar Investing.com en español (noticias generales de mercado)
  const investingFeeds = [
    "https://es.investing.com/rss/news_25.rss",   // Noticias de acciones
    "https://es.investing.com/rss/news_285.rss",  // Economía
    "https://es.investing.com/rss/news.rss"       // General
  ];
  for (const feed of investingFeeds) {
    const xml = await httpsGet(feed, headers);
    if (xml && xml.includes("<item>")) {
      const items = parseRSS(xml, 8, "Investing.com");
      if (items.length) return items;
    }
  }
  // 2) Respaldo: Yahoo Finance en inglés
  const yfFeed = ticker
    ? `https://feeds.finance.yahoo.com/rss/2.0/headline?s=${encodeURIComponent(ticker)}&region=US&lang=en-US`
    : `https://feeds.finance.yahoo.com/rss/2.0/headline?s=^GSPC,^DJI,^IXIC&region=US&lang=en-US`;
  const xml = await httpsGet(yfFeed, headers);
  if (xml && xml.includes("<item>")) return parseRSS(xml, 8, "Yahoo Finance");
  return [];
}
...
function buildDesc(ticker, price, ma50, ma100, ma200, rsi, signal) {
  const parts = [];
  if (ma200) {
    const pct = Math.abs(((price - ma200) / ma200) * 100).toFixed(1);
    if (price > ma200) parts.push(`El precio está ${pct}% por encima de su promedio de los últimos 200 días — señal de tendencia positiva.`);
    else parts.push(`El precio está ${pct}% por debajo de su promedio de los últimos 200 días — el activo lleva tiempo bajando.`);
  }
  if (ma50) {
    if (price > ma50) parts.push(`En las últimas semanas el precio sigue subiendo.`);
    else parts.push(`En las últimas semanas el precio ha estado bajando.`);
  }
  if (rsi <= 30) parts.push(`El activo ha caído mucho y rápido — puede ser una oportunidad de compra, pero también podría seguir bajando.`);
  else if (rsi >= 70) parts.push(`El activo ha subido mucho en poco tiempo — existe riesgo de que corrija hacia abajo pronto.`);
  else if (rsi >= 50) parts.push(`El impulso es moderadamente positivo, sin señales extremas.`);
  else parts.push(`El impulso es moderadamente negativo, sin señales extremas.`);
  if (signal === "buy") parts.push(`En resumen: buen momento para considerar este activo.`);
  else if (signal === "caution") parts.push(`En resumen: no es el mejor momento para entrar. Conviene esperar.`);
  else parts.push(`En resumen: el activo no muestra una dirección clara. Mejor observar antes de decidir.`);
  return parts.join(" ");
}
...
function detectLevels(highs, lows, closes, price) {
  ...
  function findLocalMins(arr, window = 5) {
    ...
    for (let i = window; i < arr.length - window; i++) {
      ...
      if (left.every(v => v >= val) && right.every(v => v >= val)) mins.push({ idx: i, price: val });
    }
    return mins;
  }
  function findLocalMaxs(arr, window = 5) {
    ...
    for (let i = window; i < arr.length - window; i++) {
      ...
      const left  = arr.slice(i - window, i).filter(Boolean);
      const right = arr.slice(i + 1, i + window + 1).filter(Boolean);
      if (left.every(v => v <= val) && right.every(v => v <= val)) maxs.push({ idx: i, price: val });
    }
    return maxs;
  }
  function clusterLevels(points) {
    ...
    for (let i = 1; i < sorted.length; i++) {
      ...
      if ((sorted[i].price - prev.price) / prev.price < 0.02) current.push(sorted[i]);
  ...
  if (support && resistance) {
    ...
    if (distToSupport <= 3) explanation = `El precio está muy cerca de una zona donde históricamente ha rebotado hacia arriba.`;
    else if (distToResistance <= 3) explanation = `El precio se está acercando a una zona donde antes encontró dificultades para seguir subiendo.`;
    else explanation = `El precio está entre dos zonas importantes. La zona de compra está ${distToSupport}% más abajo y la zona de cautela ${distToResistance}% más arriba.`;
  } else if (support) {
  ...
  return {
    support: support ? { low: Math.round(support.price*(1-ZONE)*100)/100, high: Math.round(support.price*(1+ZONE)*100)/100, center: Math.round(support.price*100)/100, touches: support.touches } : null,
    resistance: resistance ? { low: Math.round(resistance.price*(1-ZONE)*100)/100, high: Math.round(resistance.price*(1+ZONE)*100)/100, center: Math.round(resistance.price*100)/100, touches: resistance.touches } : null,
...
function scoreExplain(score, zone, price, ma50, ma100, ma200, rsi, dividendYield) {
  const yp = Math.round((dividendYield || 0) * 1000) / 10;
  const divText = yp >= 1 ? ` Además paga ${yp}% de dividendos al año.` : "";
  if (score >= 80) return `Este activo tiene muy buenas condiciones ahora mismo.${divText}`;
  if (score >= 60) return `Este activo luce interesante — vale la pena seguirlo de cerca.${divText}`;
  if (score >= 40) return `Este activo está en zona neutral.${divText}`;
  return `Este activo requiere precaución.${divText}`;
}
...
async function getTickerData(ticker) {
  ...
  const chg = rawChg != null ? Math.round(rawChg * 10) / 10 : calcChange(closes);
  const ma20  = calcSMA(closes, 20);
  const ma50  = calcSMA(closes, 50);
  const ma100 = calcSMA(closes, 100);
  const ma200 = calcSMA(closes, 200);
  const rsi = calcRSI(closes, 14);
  const cpSignal = (ma20 && ma50) ? (ma20 > ma50 ? 'alcista' : 'bajista') : null;
  const lpSignal = (ma50 && ma200) ? (ma50 > ma200 ? 'golden' : 'death') : null;
  ...
  return {
    ticker: ticker.toUpperCase(), name, price, chg, rsi,
    ...
    score, scoreZone: zone, scoreDesc, levels,
  };
}
...
const CATEGORIES = {
  etf: ['VOO','SPY','QQQ','VTI','SCHD','QQQM','VGT','SOXX','GLD','IEF','VEA','VWO','ARKK','SCHG','VYM','VIG','RSP','IWF','VNQ','TLT','HYG','BND','EFA','EEM'],
  tech: ['AAPL','MSFT','NVDA','META','GOOGL','AMZN','TSLA','AMD','AVGO','ORCL','CRM','ADBE','INTC','QCOM','NOW'],
  dividends: ['KO','JNJ','PG','VZ','T','MO','PFE','XOM','CVX','JPM','HD','MCD','WMT','MMM','IBM'],
  international: ['VEA','VWO','EFA','EEM','VXUS','IEMG','VGK','VPL','EWJ','FXI','INDA','EWZ','MCHI','VT','ACWI']
};

function categorizeTickers(tickers) {
  const result = { etf:[], tech:[], dividends:[], international:[], popular:[] };
  for (const t of tickers) {
    if (CATEGORIES.etf.includes(t)) result.etf.push(t);
    else if (CATEGORIES.tech.includes(t)) result.tech.push(t);
    else if (CATEGORIES.dividends.includes(t)) result.dividends.push(t);
    else if (CATEGORIES.international.includes(t)) result.international.push(t);
    else result.popular.push(t);
  }
  const defaults = {
    etf: ['VOO','SPY','QQQ','VTI','SCHD'],
    tech: ['AAPL','MSFT','NVDA','META','GOOGL'],
    dividends: ['SCHD','VYM','VIG','KO','JNJ'],
    international: ['VEA','VWO','EFA','EEM','VXUS']
  };
  for (const cat of ['etf','tech','dividends','international']) {
    if (result[cat].length < 5) {
      for (const t of defaults[cat]) {
        if (!result[cat].includes(t)) result[cat].push(t);
        if (result[cat].length >= 5) break;
      }
    }
...
const server = http.createServer(async (req, res) => {
  ...
  if (parsed.pathname === "/news") {
    try {
      const ticker = parsed.query.ticker || null;
  ...
  if (parsed.pathname === "/trending") {
    try {
      const data = await fetchTrending();
      const quotes = data?.finance?.result?.[0]?.quotes || [];
      const tickers = quotes.map(q => q.symbol).filter(t => t && !t.includes('=') && !t.includes('^'));
      const categorized = categorizeTickers(tickers);
      res.writeHead(200);
